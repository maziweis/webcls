<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Y14.aspx.cs" Inherits="SmarterClassroomWeb.PreLesson.PreviewPage.Chinese.Y14" %>
        ...
        $(function () {
            ...
            $.post(Constant.SunnyTask + "TaskApi/Question/GetQuestionInfo", obj, function (data) {
                if (data.Success) {
                    ...
                    //题目Html
                    var questionHtml = "";
                    questionHtml += '<table><tr>';
                    $.each(question.SelectList, function (index, value) {
                        if (IsDo) {
                            if (value.Sort == question.StuAnswer.Answer) {
                                questionHtml += '<td><a href="javascript:void(0)" class="on" onclick="Click(this)" id="' + value.Sort + '"><em class="em">' + String.fromCharCode(64 + parseInt(value.Sort)) + '</em><span>' + value.SelectItem.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>") + '</span></a></td>';
                            } else {
                                questionHtml += '<td><a href="javascript:void(0)" onclick="Click(this)" id="' + value.Sort + '"><em class="em">' + String.fromCharCode(64 + parseInt(value.Sort)) + '</em><span>' + value.SelectItem.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>") + '</span></a></td>';
                            }
                        } else {
                            questionHtml += '<td><a href="javascript:void(0)" onclick="Click(this)" id="' + value.Sort + '"><em class="em">' + String.fromCharCode(64 + parseInt(value.Sort)) + '</em><span>' + value.SelectItem.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>") + '</span></a></td>';
                        }
