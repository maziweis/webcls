<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="M11.aspx.cs" Inherits="SmarterClassroomWeb.PreLesson.PreviewPage.English.M11" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            //M11 调试路径
            //http://192.168.3.94:8010/QuestionModels/M11.aspx?QuestionID=0067A3AA-0F26-4F19-B751-DBADFECA630C&AccessType=3&StuTaskID=24edcdb3-d172-4418-8262-0c2dcac8f436
            ...
            $.post(Constant.SunnyTask + "TaskApi/Question/GetQuestionInfo", obj, function (data) {
                if (data.Success) {
                        ...
                        $.each(data.Data, function (i) {
                            if (this.ParentID != null && this.ParentID != "") {
                                for (var i = 0; i < this.BlankAnswer.length; i++) {
                                    if (maxLen < this.BlankAnswer[i].Answer.length) {
                                        maxLen = this.BlankAnswer[i].Answer.length;
                                    }
                                }
                            }
                        });
                    ...
                    $("input:text").keydown(function (event) {
                        //判断输入的是空格
                        if (event.keyCode == 32) {
                            var curIndex = Common.getCursortPosition(this);
                            //光标在最前面
                            if (curIndex == 0) {
                                return false;
                            }
                                ...
                                //获取光标前的字符
                                var beforeCur = this.value.charAt(curIndex - 1);
                                //若字符为空格
                                if ($.trim(beforeCur) == "") {
                                    return false;
                                }
                                //获取光标后的字符
                                var afterCur = this.value.charAt(curIndex);
                                //若光标不在最后，afterCur不能为空格
                                if (curIndex != this.value.length && $.trim(afterCur) == "") {
                                    return false;
                                }
                            }
                        }
                        if (event.keyCode <= 87 && event.keyCode >= 65 && this.value.length > maxLen + 5) {
                            alert("输入字符长度超出可输入范围，请检查！");
                            return false;
                        }
                    });
                }
        ...
        function AnswerSave(txtId) {
            if ($("#" + txtId) != undefined) {
                ...
                var stuAnswer = $.trim($("#" + txtId).val().replace("/^\s+|\s+$/", ""));

                if (AnswerList[AnsweIndex - 1] != null) {  //当前小题有答题记录,直接保存
                    if (stuAnswer != AnswerList[AnsweIndex - 1]) {
                        //与历史答案不一致，重新保存
                        var spendTime = new Date() - startTime;
                        var Isright = (stuAnswer == BlankAnswer[AnsweIndex - 1] ? 1 : 0);
        ...
        function QueWrongAnswer() {
            ...
            $.each(allText, function (i) {
                ...
                if ($.trim($(this).val()).replace("/^\s+|\s+$/", "") != BlankAnswer[i]) {
                    hanError = 1;
