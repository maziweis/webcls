<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="M6.aspx.cs" Inherits="SmarterClassroomWeb.PreLesson.PreviewPage.Js.M6" %>
            ...
            $.post(Constant.SunnyTask + "TaskApi/Question/GetQuestionInfo", obj, function (data) {
                if (data.Success) {
                    ...
                    $.each(question.SelectList, function (index, value) {
                        if (value.SelectItem.length > maxLen)
                        {
                    ...
                    $.each(question.SelectList, function (index, value) {
                        ...
                        if (IsDo) {
                            if (value.Sort == question.StuAnswer.Answer) {
                                questionHtml += '<a href="javascript:void(0)" class="on" onclick="Click(this)" id="' + value.Sort + '">';
                            } else {
                                questionHtml += '<a href="javascript:void(0)" onclick="Click(this)" id="' + value.Sort + '">';
                            }
                        } else {
                            questionHtml += '<a href="javascript:void(0)" onclick="Click(this)" id="' + value.Sort + '">';
                        }
                        ...
                        if (maxLen > 27) {
                            questionHtml += '<span style="width:700px;">' + value.SelectItem.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>") + '</span></a>';
                        } else {
                            questionHtml += '<span>' + value.SelectItem.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>") + '</span></a>';
                        }
                        questionHtml += '</td>';
