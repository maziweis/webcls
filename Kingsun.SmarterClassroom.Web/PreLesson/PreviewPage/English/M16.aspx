<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="M16.aspx.cs" Inherits="SmarterClassroomWeb.PreLesson.PreviewPage.English.M16" %>
...
<body>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            //AccessType = 2;
            //M16 调试地址
            //http://192.168.3.94:8010/QuestionModels/M16.aspx?QuestionID=6A23EF2B-9B7F-4A1A-B94F-17929C9E5DC6&AccessType=2&StuTaskID=64549506-de0d-4400-a647-ec4c06a0b462
            ...
            $.post(Constant.SunnyTask + "TaskApi/Question/GetQuestionInfo", obj, function (data) {
                if (data.Success) {
                    ...
                    if (data.Data.length != undefined) {
                        window.parent.HidePlay(); //隐藏父页面播放控件
                        var ImgHtml = ""; //填充图片
                        ...
                        $.each(data.Data, function (i) {
                            if (this.ParentID != null && this.ParentID != "") {
                                ...
                                ImgHtml += "<li onclick=\"ImgClick('" + queNum + "')\"><a href=\"javascript:void(0)\">" +
                                        "<img src=\"" + this.ImgUrl + "\" alt=\"\" /><em class=\"em\">" + (sub == "S" ? String.fromCharCode(64 + queNum) : queNum) + "</em></a></li>";

                                var stuAnswer = "";
                                if (this.StuAnswer != undefined) {
                                    stuAnswer = "value=\"" + (sub == "S" ? String.fromCharCode(64 + parseInt(this.StuAnswer.Answer)) : this.StuAnswer.Answer) + "\"";
                                    IsDo = true;
                                }
                                //<td><input type="text" value="" /><span>book</span></td>
                                AnswerHtml += "<td><input class=\"\" name=\"txtAnswer\" num=\"" + queNum + "\" id=\"" + this.QuestionID
                                              + "\" onclick=\"AnswerTextClick('" + queNum + "')\" type=\"text\" "
                                              + stuAnswer + " maxlength=\"1\" readonly=\"readonly\" QueAnswer=\"" + this.BlankAnswer[0].Answer + "\"/>"
                                if (this.QuestionModel.substring(0, 1) == "Y") {
                                    AnswerHtml += "<span style=\"font-family:GB Pinyinok-C\">";
                                } else {
                                    AnswerHtml += "<span>";
                                }
                                AnswerHtml += this.QuestionContent.replace(/\\n/g, '<br/>').replace(/\/n/g, '<br/>') + "</span></td>";//
                                if (queNum % 2 == 0) {
                                    AnswerHtml += "</tr><tr>";
                                }
                                ...
                                if (this.BlankAnswer[0].Answer != undefined) {
                                    ...
                                    //BlankAnswer[queNum - 1] = this.BlankAnswer[0].Answer;
        ...
        function ImgClick(ImgNum) {
            ...
            if (allText[AnsweIndex - 1] != undefined) { //判断当前答案输入框是否存在
                var Answer = sub == "S" ? String.fromCharCode(64 + parseInt(ImgNum)) : ImgNum;
                $(allText[AnsweIndex - 1]).val(Answer).removeClass(); //赋值点击的序号，同时移除答案框上的样式
                //此处判断是否要保存答案 或显示正确答案
                if (AccessType == 3) { //学生做题，实时保存答案
                    ...
                    //var obj = { QuestionID: qid, StuTaskID: StuTaskID, Answer: ImgNum, Isright: Isright, spendTime: spendTime, ParentID: ParentID };
                    ...
                    var minQAnswer = {
                        ...
                        Answer: ImgNum, IsRight: Isright, StuScore: stuScore, SpendTime: spendTime
                    };
                    var parentQAnswer = {
        ...
        function QueWrongAnswer() {
            ...
            var tmpAnswer = "";
            $.each(allText, function (i) {
                tmpAnswer = (sub == "S" ? String.fromCharCode(64 + parseInt(BlankAnswer[i])) : BlankAnswer[i]);
                if ($(this).val() != tmpAnswer) {
                    hanError = 1;
                    answerHtml += "<span class=\"w\">" + tmpAnswer + "</span>" + "   ";
                }
                else {
                    answerHtml += "<span class=\"r\">" + tmpAnswer + "</span>" + "   ";
                }
            });
