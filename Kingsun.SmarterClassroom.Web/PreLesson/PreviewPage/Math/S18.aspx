<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="S18.aspx.cs" Inherits="SmarterClassroomWeb.PreLesson.PreviewPage.Math.S18" %>
    ...
    <script type="text/javascript">
                    ...
                    for (var i = 0; i < data.Data.length; i++) {
                        if (data.Data[i].ParentID == "") {
                            ...
                            if (data.Data[i].QuestionContent) {
                                hasContent = true;
                                $(".stemBox").html('<div class="titBox">' + Common.MatchFenShi(data.Data[i].QuestionContent.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>")) + '</div>');
                            } else {
                                hasContent = false;
                            }
                        ...
                        //获取小题的内容
                            ...
                            questionHtml += '<span class="sp">' + Common.MatchFenShi(data.Data[i].QuestionContent.replace(/\\n/g, "<br/>").replace(/\/n/g, "<br/>").replace("[", "<u>").replace("]", "</u>")) + '</span></h3>';
                                ...
                                if (isDo) {
                                    ...
                                    if (data.Data[i].StuAnswer.Answer == value.Sort) {
                                        questionHtml += '<td><a href="javascript:void(1)" class="on" onclick="liClick(this)" id="' + data.Data[i].QuestionID + '_' + value.Sort + '_' + data.Data[i].Sort + '"><em class="em">'
                                            + (String.fromCharCode(64 + value.Sort)) + '</em><span class="MathSp">'
        ...
        function resizeWindow() {
            if (hasContent) {
                ...
                $('.S .stem').niceScroll({ touchbehavior: false, autohidemode: false, cursorcolor: "#dcdcdc", cursoropacitymax: 1, cursorwidth: 8 });
                $('.S .problem').niceScroll({ touchbehavior: false, autohidemode: false, cursorcolor: "#dcdcdc", cursoropacitymax: 1, cursorwidth: 8 });
            } else {
        ...
        function liClick(obj) {
            ...
            $.each($("#" + (currentID.split('_')[0]) + " a"), function (val) {
                $("#" + (this.id)).removeClass();
            });
            $(obj).addClass("on");
            ...
            if (AccessType == 3) {
                ...
                startTime = new Date();
                SaveAnswer(currentID, spendTime);
            }
        }
