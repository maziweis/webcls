var PreLessonForTSInit = function () {
    ...
    this.InitUserResource = function () {
        ...
        preLessonCom.GetCatalogListByBookID(Current.SelectBookID, Current.CataList, function (data) {
            Current.CataList = data; $("#unitname").html(Current.CataList[0].title);
            Current.CataID = Current.CataList[0].id;
            var html = "";
            $.each(Current.CataList, function (u, unit) {
                var name = unit.title;
                html += '<option value="' + unit.id + '">' + name + '</option>';
    ...
    this.InitSyncResourceCata = function () {
        Common.GetCatalogByBookId(Current.SelectBookID, function (data) {
            ...
            $("#selectSyncCatalog").change(function () {
                Current.PageIndex = 0;
                /////////////加载两级目录/////////////////
                if (cataList[0].title.indexOf("Unit") == -1 || (Current.SubjectID != 3 && typeof (cataList[0].children) != "undefined")) {
                    Current.SelectCataID = $(this).children('option:selected').val();
                    $("#selectSyncUnit").show();
            ...
            if (now_cata == null) {
                if (Current.SelectCataID != "") {
                    ...
                    $('#selectSyncCatalog').val(cataList[0].id).change();
                }
            }
            else {
                Current.SelectUnitID = Current.SelectCataID;
    ...
    this.InitCatalog = function () {
        ...
        preLessonCom.GetCatalogListByBookID(Current.BookID, Current.CataList, function (data) {
            Current.CataList = data;
            Current.CataID = Current.CataList[0].id;
            $("#unitname").html(Current.CataList[0].title);
            $("#unitFull").text(Current.CataList[0].title);
            InitPage();
            return false;
    ...
    this.BindCataClick = function () {
        $("#unitname li").on("click", function () {
            ...
            if (Current.CataID == li.attr("id") || Current.CataID == "")
                Current.IsSame = true;
            savemap();  //更新教学地图的session
            loadMap();//加载网站地图内容
            if (Current.UserFinallyOperationRecord != null) {
                if (Current.CataID != Current.UserFinallyOperationRecord.UnitID) {
                    if (isOrNotOperation()) {
                        //////////检查是否有页面cookies/////////////
    ...
    this.ChangeInit = function (li) {
        ...
        $(".pageList h3.currentName label").text(txtname);
        slideAnimate("close");
    ...
    this.preview = function () {
        ...
        var pageIndex = $("#pageBox ul li a.cur").attr("hidpage");
        var href = 'LessonView.html?UserID=' + Current.UserID + "&BookID=" + bookID + "&BookType=" + BookType + "&CataID=" + Current.SelectUnitID + "&page=" + pageIndex + "&SubjectID=" + getUrlParam("SubjectID");
        if (BookType == 6) {
            href = 'LessonViewPD.html?UserID=' + Current.UserID + "&BookID=" + bookID + "&BookType=" + BookType + "&CataID=" + Current.SelectUnitID + "&page=" + pageIndex + "&SubjectID=" + getUrlParam("SubjectID");
        }
