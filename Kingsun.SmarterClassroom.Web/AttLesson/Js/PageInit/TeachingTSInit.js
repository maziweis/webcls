...
var ResourceUrl = 'http://192.168.3.2:1777/FileDown.aspx?ResourceID=';
...
var TeachingInit = function () {
    ...
    this.InitCatalog = function () {
        ...
        Common.GetCatalogByBookId(Current.BookID, function (data) {
            if (data.length == 0)
                return;
                Current.UnitID = data[0].id;
            Current.InitTeachMap();
            var cataList = data;
            $("#unitFull").text(data[0].title);
            $.getScript(Current.JsonJS, function (data) {
                var pageTitleStr = '<li href="javascript:void(0)" hidpage="' + 2 + '" ' + ' title="' + cataList[0].title + '" onclick="switchPaget(' + 2 + ',this)">' + cataList[0].title + '</li>';
                var jsonObj = JSON.parse(dbJson);
                $(jsonObj.pageSource).each(function (index, items) {                   
                    //加载目录
                    if (items.pageImg.indexOf("page") != -1) {
                        if (items.pageId == Current.PageNum) {
                            classStr = 'class="on"';
                        }
                        else {
                            classStr = '';
                        }
                        pageTitleStr += '<li href="javascript:void(0)" hidpage="' + items.pageId + '" ' + classStr + ' title="第' + items.pageId + '页" onclick="switchPaget(' + items.pageId + ',this)">第' + items.pageId + '页</li>';
                    }
                });
                $(".catalogOl").html(pageTitleStr);
                //用户第一次使用默认保存一次数据(当前第一单元第一页)
                if (SelTeachingData == null || SelTeachingData.length==0) {
                    var CreateTime = commonFuncJS.getNowFormatDate();
                    PageInitID = commonFuncJS.GetGuid();
                    teachLessonManage.SaveTeachingData(PageInitID, UserID, Current.EditionID, Current.SubjectID, Current.GradeID, Current.ClassID, Current.BookID, 0, 2, AspxName, CreateTime, $("#unitFull").text());
                }
            });
            curUnitName = $("#unitFull").text();
        });
    };
};
...
function Update(UnitID, PageStart) {
    var CreateTime = commonFuncJS.getNowFormatDate();
    teachLessonManage.UpdateTeachingData(UserID, teachingInit.EditionID, teachingInit.SubjectID, teachingInit.GradeID, teachingInit.ClassID, teachingInit.BookID, UnitID, PageStart, AspxName, CreateTime, $("#unitFull").text());
}
var Resources;
